...
export const SettingsModal: React.FC = () => {
  ...
  const isAdminUser = currentUser.email.toLowerCase() === 'sparshchauhan050@gmail.com';
          ...
          <div className="space-y-6">
            ...
            <div className="p-5 rounded-2xl border space-y-4 shadow-md" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
              <div className="flex items-center justify-between">
                ...
                {isAdminUser ? (
                  <span className="text-[11px] text-emerald-400 font-bold flex items-center gap-1">
                    <CheckCircle2 className="w-3.5 h-3.5" /> Full Admin Clearance
                  </span>
                ) : (
              ...
              <p className="text-xs opacity-70">
                {isAdminUser
                  ? "You are logged in as the platform administrator (Sparsh Chauhan). You have full administrative rights across channels, meetings, and analytics."
                  : "Administrative and Owner privileges are restricted exclusively to Sparsh Chauhan (sparshchauhan050@gmail.com)."}
              </p>

              <div className="flex flex-wrap items-center gap-3 pt-2">
                {(['owner', 'admin', 'moderator', 'member'] as const).map(role => {
                  const isRoleLocked = (role === 'owner' || role === 'admin') && !isAdminUser;
                  return (
                    <button
                      key={role}
                      onClick={() => !isRoleLocked && switchUserRole(role)}
                      disabled={isRoleLocked}
                      className={`px-3.5 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider border transition-all flex items-center gap-1.5 ${
                        currentUser.role === role
                          ? 'bg-accent text-white border-accent shadow-md shadow-accent/20'
                          : isRoleLocked
                          ? 'opacity-30 cursor-not-allowed bg-black/10'
                          : 'opacity-70 hover:opacity-100 hover:border-accent'
                      }`}
                      title={isRoleLocked ? 'Admin roles are restricted to sparshchauhan050@gmail.com' : `Switch to ${role}`}
                    >
                      {isRoleLocked && <Lock className="w-3 h-3" />}
                      <span>{role}</span>
                    </button>
                  );
                })}
