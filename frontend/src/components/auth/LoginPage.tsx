...
export const LoginPage: React.FC = () => {
  ...
  return (
    <div
      className="min-h-screen w-screen flex items-center justify-center p-4 font-sans select-none relative overflow-hidden transition-colors duration-300"
    ...
    >
      ...
      {/* Main Auth Card */}
      <div
        className="w-full max-w-md rounded-3xl p-8 border shadow-2xl relative z-10 space-y-6"
      ...
      >
        ...
        {activeTab === 'login' ? (
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            <div>
              <label className="text-xs font-semibold opacity-80 block mb-1.5">Email Address</label>
          ...
          <form onSubmit={handleSignUpSubmit} className="space-y-3">
            ...
            <div>
              <label className="text-xs font-semibold opacity-80 block mb-1">Work Email</label>
              <div className="relative flex items-center">
                <Mail className="w-4 h-4 opacity-50 absolute left-3.5 text-accent" />
                <input
                  type="email"
        ...
        <div className="pt-3 border-t space-y-2" style={{ borderColor: 'var(--border-color)' }}>
          <span className="text-[10px] uppercase font-bold opacity-60 block text-center">Quick Demo Switcher</span>
          <div className="grid grid-cols-3 gap-2">
            <button
              onClick={() => handleDemoLogin('Alex Rivera', 'alex.rivera@letsconnect.io', 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80')}
            ...
            >
              <UserCheck className="w-3 h-3 text-accent" /> Alex (Owner)
            </button>
            <button
              ...
              style={{ backgroundColor: 'var(--bg-primary)', borderColor: 'var(--border-color)' }}
            >
              <UserCheck className="w-3 h-3 text-accent" /> Sarah (Admin)
            </button>
            <button
              ...
              style={{ backgroundColor: 'var(--bg-primary)', borderColor: 'var(--border-color)' }}
            >
              <UserCheck className="w-3 h-3 text-accent" /> Marcus (Dev)
            </button>
          </div>
        </div>
      </div>
